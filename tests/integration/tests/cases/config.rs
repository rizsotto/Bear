...
/// Test header-entry synthesis with the `include-dirs` strategy.
/// Verifies that a header reachable only through a donor's own `-I` include
/// directory (not the source's own directory) receives a synthesized entry,
/// with the header's `file` path reflecting the include-dir prefix.
// Requirements: output-header-entries
#[test]
#[cfg(has_preload_library)]
#[cfg(all(has_executable_compiler_c, has_executable_shell))]
fn header_entries_include_dirs_config() -> Result<()> {
    let env = TestEnvironment::new("header_entries_include_dirs")?;

    // A compiled source in `src/`, and a header reachable only through the
    // `-Iinclude` user include directory (not a sibling of `src/main.c`).
    env.create_source_files(&[
        ("src/main.c", "int main() { return 0; }"),
        ("include/util.h", "int util(void);"),
    ])?;

    let build_commands = format!("{} -c src/main.c -Iinclude -o src/main.o", COMPILER_C_PATH);
    let script_path = env.create_shell_script("build.sh", &build_commands)?;
