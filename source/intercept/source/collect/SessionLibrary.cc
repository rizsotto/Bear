...
namespace ic {
    ...
    Session::Ptr LibraryPreloadSession::from(const Configuration& config)
    {
        return std::make_shared<LibraryPreloadSession>(config.verbose, config.library.c_str(), config.wrapper.c_str());
    }
