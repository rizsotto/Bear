...
def capture(args, tools):
    ...
    with temporary_directory(prefix='intercept-') as tmp_dir:
        ...
        output = os.path.join(tmp_dir, 'commands.json')
        command = build_command(args, output)
        exit_code = run_build(command)
        # read the intercepted exec calls
        safe_calls = parse_intercept_report(output)
...
def parse_intercept_report(filename):
    # type: (str) -> Iterable[Execution]
    """ Parse the execution report file. """
