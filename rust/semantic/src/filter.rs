...
impl EntryPredicateBuilder {
    ...
    pub fn filter_duplicate_entries(fields: Vec<EntryField>) -> Self {
        let hash_function = EntryField::hash_from(fields);
...
impl EntryField {
    fn hash_from(fields: Vec<EntryField>) -> impl Fn(&Entry) -> u64
    {
        move |entry: &Entry| {
            ...
            // return true if the hash is not in the set.
            hash
        }
    }
}
...
mod test {
    ...
    fn test_duplicate_detection_works() {
        ...
        let sut: EntryPredicate = EntryPredicateBuilder::filter_duplicate_entries(vec![
            EntryField::File,
            EntryField::Output,
        ]).build();
        let result: Vec<Entry> = input.into_iter().filter(sut).collect();
